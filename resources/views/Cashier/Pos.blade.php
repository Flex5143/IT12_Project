    ...
    <script>
        ...
        function renderMenuItems() {
            ...
            container.innerHTML = filteredItems.map(item => `
                <div class="card menu-item bg-white rounded-xl p-4 shadow-md hover:shadow-lg transition cursor-pointer ${item.stock === 0 ? 'opacity-50' : ''}" 
                     onclick="${item.stock === 0 ? '' : `addToOrder(${item.id})`}">
                    <div class="text-4xl mb-2 text-center">🍽️</div>
                    <h3 class="font-semibold text-center mb-1" style="color: #1f2937;">${item.name}</h3>
                    <p class="text-sm text-center text-gray-600 mb-2">${item.category}</p>
                    <div class="flex justify-between items-center">
                        <span class="font-bold" style="color: #dc2626;">₱${parseFloat(item.price).toFixed(2)}</span>
                        <span class="text-xs ${item.stock < 10 ? 'text-red-600' : 'text-gray-600'}">Stock: ${item.stock}</span>
                    </div>
                    ${item.stock === 0 ? '<p class="text-xs text-red-600 text-center mt-2">Out of Stock</p>' : ''}
                </div>
            `).join('');
        }
